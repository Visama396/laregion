...
export const RepartoItem = ({address, monday, tuesday, wednesday, thursday, friday, saturday, sunday, magazine, off, help}: Props) => {
  return (
    <tr className={`${(off)? 'bg-red-400' : 'bg-white'} ${(off)? 'dark:bg-red-800' : 'dark:bg-gray-800'}`}>
      ...
      <td className="px-6 py-4 text-center">
        {magazine && (
          <input type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-200 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" checked disabled />
        )}
      </td>
		</tr>
  )
