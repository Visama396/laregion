...
const fetchRepartidores = async () => {
  return await databases.listDocuments("666e1f2b000da5f62f9e", "66758ccc000cff6f85e9").catch(error => { throw new Error(error) })
}

export const AddAddress = () => {
  const [repartidores, setRepartidores] = useState([])

  fetchRepartidores().then(res => setRepartidores(res.documents))
        ...
        <form action="#" onSubmit={handleSubmit}>
          <div className="grid gap-4 sm:grid-cols-7 sm:gap-6">
              ...
              <div className="sm:col-span-4">
                <label htmlFor="repartidor" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Repartidor</label>
                <select id="repartidor" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value={formData.repartidor} onChange={(e) => setFormData({...formData, repartidor: e.target.value})} required>
